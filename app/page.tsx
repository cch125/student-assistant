...
type Answer = { ok: boolean; answer: string; documentName?: string; similarity?: number; sourceUrl?: string; matches?: { documentName: string; similarity: number; snippet: string }[] };

export default function AssistantPage() {
  const [question, setQuestion] = useState(""); const [image, setImage] = useState<{ base64: string; mime: string } | null>(null); const [answer, setAnswer] = useState<Answer | null>(null); const [loading, setLoading] = useState(false); const [configured, setConfigured] = useState(false);
  useEffect(() => { const value = loadConnection(); setConfigured(Boolean(value.baseUrl && value.apiKey && value.datasetId)); }, []);
  ...
  return <main className="page"><div className="page-grid">
    <section className="panel"><h1>学生事务查询</h1><p className="lede">检索学校官方资料、通知、办事流程、表格与附件。没有可靠材料时，助手会明确拒答。</p>
      ...
      {!configured && <div className="status-box error">尚未配置个人知识库。<Link href="/settings">前往连接与导入</Link></div>}
      <div className="answer"><h2>回答</h2>{!answer?<p className="muted">查询结果会显示在这里。</p>:<><p className={`answer-text ${answer.ok?"":"error"}`}>{answer.answer}</p>{answer.ok&&<div className="source"><span>{answer.documentName} · 相似度 {Number(answer.similarity||0).toFixed(3)}</span>{answer.sourceUrl&&<a href={answer.sourceUrl} target="_blank" rel="noreferrer">查看官方来源 <ExternalLink size={14}/></a>}</div>}{answer.matches?.slice(0,3).map((item,index)=><div className="match" key={`${item.documentName}-${index}`}><strong>{item.documentName} · {item.similarity.toFixed(3)}</strong><div>{item.snippet}</div></div>)}</>}</div>
    </section>
